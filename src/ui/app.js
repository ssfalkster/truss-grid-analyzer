...
(function (g) {
  ...
  var DOCS = [
    ...
    "<li><b>Cross-check</b>: <i>Export whole-rig model</i> (under the hoist table) saves the rig's whole-rig model with this app's results; <code>python tools/pynite_check.py file</code> rebuilds it in PyNite, the 3D frame engine behind CalcForge, and compares hoist reactions and member forces (without shear deformation, which PyNite doesn't model). If the whole-rig analysis can't run (a load-path loop, or a very large rig) the app says so and falls back to the load-path method.</li>",
  ...
  function legend() {
    ...
    if (mode === "layer") items = [["l1", "1 - carries no other truss"], ["l2", "2 - carries a 1"], ["l3", "3"], ["l4", "4+"]];
    else items = mode === "status" ? [["ok", "OK"], ["fail", "Overloaded or unstable"]] : [["ok", (mode === "hoist" ? "Hoist workload" : "Workload") + " low (under 80%)"], ["warn", "High (80-100%)"], ["fail", "Overloaded (over 100%)"]];
