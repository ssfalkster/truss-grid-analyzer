...
(function (g) {
  ...
  var DOCS = [
    ...
    "<li><b>Stiffness solve (primary, since 1.3.0)</b>: the load-path method treats every truss that carries another as an unyielding support. That is exact when the carrying truss is held up at each connection, but if a truss with its own hoist is bolted to a carrier that has no hoist near the connection, the carrier sags and the bolted truss sheds load to its own hoist, which the load-path method under-estimates (in a test rig by more than 2 times). So the app solves the whole rig with the grillage method: a matrix (direct-stiffness) analysis of the box as beams meeting at the corner blocks, the standard technique for a bolted grid such as a bridge deck. Hoist loads, the forces in the bolted connections, and so the loads each span and cantilever check sees, all come from this solve. Since no manufacturer publishes a stiffness for the corner-block hardware itself, two joint models bracket it and are shown side by side - hinged (the bolt carries vertical force only) and rigid (bending and torsion also pass through the block, the textbook grillage assumption, and the one an independent 3D frame analysis of a similar grid also implies). Every check uses the worse of the two: a hoist's total static, dynamic load and status use the larger load, and each truss is checked with the joint model that loads it harder (named on the truss). Each truss's bending (EI) and torsional (GJ) stiffness is estimated from its published size and its connector type (plated/bolted trusses are more flexible than spigoted ones of the same size, and lose more of their torsional stiffness through a bolted joint); scale a truss's estimate with its Stiffness (x) field. Hoists are treated as rigid supports. If the stiffness solve can't run (a load-path loop, or a very large rig) the app says so and falls back to the load-path method.</li>",
