...
(function (g) {
  ...
  var DOCS = [
    ...
    "<li><b>Hoists and level sensitivity (since 1.4.0)</b>: hoists are rigid supports at one level unless you give a hoist stiffness (lb per inch of stretch, for the hoist and its chain) in Rig settings. A hoist's panel shows how much a hoist's load changes if it runs 1/4 in high (or low) - on short, stiff spans that can be hundreds of pounds, so a hoist whose level sensitivity passes 10% of its capacity is flagged: level the hoists carefully, or check with a hoist stiffness.</li>",
    ...
    "<li><b>Hoists hung below a truss (since 1.22.0)</b>: set a hoist's <i>Hangs from</i> to another truss when it hangs from that truss instead of the structure (a truss flown under a truss). The hoist hooks on where the plan puts it; its chain joins the two trusses, so the upper truss (the carrier) sags with the load and the lower truss shares load with its other hoists. The carrier carries the hung hoist's high hook load (low hook + Add % + hoist, chain and hardware weight) and is checked with its high hook <i>dynamic</i> load; so are the hoists holding the carrier up, unless Rig settings turn that off. The plan draws a hung hoist with a dashed ring; the rig's total high hook load counts only hoists to the structure.</li>" +
    "<li><b>Dead hangs (since 1.22.0)</b>: a support that is just wire rope (and shackles) from the truss to the hang point, no hoist - set a hoist's <i>Support</i> to Dead hang (or type \"dead\" in the grid's Hoist cell). Pick the rope (7x19 galvanized aircraft cable or 6x19 IWRC, typical catalogue breaking strengths) and its length; its WLL is the breaking strength / the design factor in Rig settings (7, 8 or 10:1, default 10:1), capped by the assembly WLL you type (the weakest shackle or fitting). No hoist body, no chain: the high hook load is the low hook + Add % + rope and hardware weight, and it is static (dynamic factor 1.0 unless typed). It is rigid like the hoists; when the rig's hoists are springs, the rope is a spring EA/L (much stiffer than a chain hoist, so it draws load). Shown as a filled dot on the plan and DH1, DH2... on the calc sheet.</li>" +
    "<li><b>Hoist levels (since 1.22.0)</b>: two separate things. A <i>Level offset</i> on a hoist (inches, + higher) is a designed trim - a truss hung raked or at a different height on purpose, or a dead hang whose length is fixed: the whole-rig analysis solves the rig with that point moved, and the loads shift accordingly. The <i>out-of-level tolerance</i> in Rig settings (e.g. ±1/4\") is unintended error: any hoist may end up that far off, each on its own, and every hoist's check carries the worst it could see (the sum of what each hoist alone does to it); the low side is reported, and a hoist that could unload completely is warned about. Both need the whole-rig analysis.</li>" +
    "<li><b>Check rig (since 1.22.0)</b>: on every change the app looks for input mistakes that the calculation would otherwise take at face value - a load with no weight, two hoists at the same point, a bolted assembly with fewer than 3 hoists or all of them in one line, bolted ends that don't meet on the plan, trusses that cross with no joint (a note: they may run one above the other), a hoist with no model. They are listed in the rig's panel (Check rig) and with the warnings; click one to select it. The truss panel's <i>Connected</i> button dims everything not joined to that truss, to spot a part that is not connected. Nothing here changes a result.</li>" +
    "<li><b>Allowances (since 1.22.0)</b>: weight the drawing doesn't show. A <i>cable allowance</i> per length (Rig settings, or a truss's own in its panel; 0 = none) is added to each truss's UDL. <i>Load factors</i> by category (lighting, audio, video, scenic, rigging hardware, other) multiply each load's weight - set a load's category in its panel or the Loads grid; fixtures from the library are lighting. Everything is calculated with the factored weights; the loads keep the weights you typed. (Eurocode partial factors are not used: the manufacturers' tables are allowable loads.)</li>" +
    "<li><b>Measured vs calculated (since 1.22.0)</b>: type what each load cell reads in the hoist grid's Measured column (or the hoist's panel). Rig settings says where the cells hang - above the hoist (compared with the high hook static load) or between hoist and truss (the low hook load). The rig panel totals each assembly of joined trusses and flags it past 5%; a single hoist on a rig that shares load can legitimately differ more (level, stiffness). Readings never change a result.</li>" +
    ...
    "<li><b>Edit many hoists at once (since 1.24.0)</b>: in the Hoists grid (step 3), Shift+click selects a range, Ctrl+click adds or removes one hoist, Shift+arrows grow the selection, Ctrl+A selects all and <i>Select</i> on a truss row selects its hoists. Any edit then changes every selected hoist (not their positions); Ctrl+D copies the top one to the rest; Delete removes them; Esc clears. A new hoist starts as a copy of the last hoist placed (model, chain or rope, hardware, dynamic factor).</li>",
    "<li><b>Mirror and copy hoists (since 1.23.0)</b>: <i>Mirror</i> (hoist panel) and <i>Mirror all</i> (truss panel, Hoists grid) add a hoist at the mirrored spot about the centerline; <i>Copy to truss</i> copies hoists to another truss at the same distance from its centre. Each copy is its own hoist (edit it separately) with the same model, chain or rope, dynamic factor, hardware, level offset and Hangs from - but no name or load-cell reading. No copy is placed where a hoist already is.</li>",
    ...
    "<li><b>Deflection view (since 1.22.0)</b>: the <i>Deflection</i> colour mode colours each truss by its worst span's sag against its limit, and draws its deflected shape from the whole-rig analysis, exaggerated with one scale for the whole rig - beside the truss on the plan (a sag is drawn to its right-hand side) and above it in 3D. Hover a shape on the plan for its largest sag. It also shows a truss that sags with its carrier or a springy hoist, and a part that is not connected.</li>" +
  ...
  function legend() {
    ...
    items = mode === "defl" ? [["ok", "Sag under 80% of its limit"], ["warn", "80%+ of the limit, or past the rig default"], ["fail", "Past the maker's published limit"], ["neutral", "No span to check"]]
      : mode === "status" ? [["ok", "OK"], ["fail", "Overloaded or unstable"]] : [["ok", (mode === "hoist" ? "Hoist workload" : "Workload") + " low (under 80%)"], ["warn", "High (80-100%)"], ["fail", "Overloaded (over 100%)"]];
    $("legend").innerHTML = items.map(function (i) { return '<div><i style="background:var(--' + i[0] + ')"></i>' + i[1] + "</div>"; }).join("") +
      '<div><svg width="14" height="14" viewBox="0 0 14 14" style="margin-right:4px;vertical-align:-3px"><path d="M0 7H14M7 0V14" stroke="var(--hoist)" stroke-width="1.6"/><circle cx="7" cy="7" r="4" fill="var(--panel)" stroke="var(--hoist)" stroke-width="1.8"/><circle cx="7" cy="7" r="1.3" fill="var(--hoist)"/></svg>Hoist: high hook static, <em>dynamic</em> (amber = high, red = overloaded)</div><div><svg width="14" height="14" viewBox="0 0 14 14" style="margin-right:4px;vertical-align:-3px"><circle cx="7" cy="7" r="4.5" fill="var(--panel)" stroke="var(--hoist)" stroke-width="1.8" stroke-dasharray="3 2"/></svg>Dashed: hoist hung below a truss</div><div><svg width="14" height="14" viewBox="0 0 14 14" style="margin-right:4px;vertical-align:-3px"><circle cx="7" cy="7" r="4.5" fill="var(--hoist)"/></svg>Dead hang (wire rope, no hoist)</div><div><i style="background:var(--ink);transform:rotate(45deg) scale(.8)"></i>Bolted connection</div>';
  }
  ...
  function dockHead(step) {
    ...
    if (step === 3) {
      d.appendChild(h("label", { "class": "chk", title: "Show each whole-rig joint model" }, h("input", { type: "checkbox", checked: TLA.grids.compare(), onchange: function (e) { TLA.grids.compare(e.target.checked); renderAll(); } }), "Compare analysis methods"));
