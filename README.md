...
it from chain hoists, add your loads, and see the load on every hoist and the workload of every span and
cantilever.
...
- **Truss lines from real pieces.** Build each truss from stock lengths and corner blocks
  ...
  Christie Lites and James Thomas Engineering catalogs.
...
- **Whole-rig analysis.** The whole rig is solved together: a direct-stiffness analysis of the grid (a
  grillage), with the trusses as Timoshenko beams sharing deflection at the joints. Corner blocks are modeled hinged,
  ...
  diagonal sizes where the truss data has them.
- **Load-path method (fallback).** The original workbook's method: each truss solved on its own as a continuous
  beam with cantilevers (Clapeyron's three-moment equation), its reactions at bolted connections passed on to the
  truss it is bolted to, down to the hoists. It is used only when the whole-rig analysis can't run, and the app
  warns that it can under-estimate hoists in a grid.
- **Level sensitivity.** For each hoist, how much its load changes if it runs 1/4 in high or low, flagged past
  10% of its capacity. Hoists can be given a stiffness (lb/in) instead of being rigid.
...
- **Loads.** Point loads from a fixture library, clamps, UDL, loads mirrored about the
...
- The whole-rig analysis is checked against PyNite, the 3D frame engine behind CalcForge's *3D Structural
  ...
  export is the Euler-Bernoulli version of the model. Timoshenko shear, level sensitivity and hoist springs are tested
  against closed-form beam results.

## Limitations

- Hoists are rigid supports at one level unless a hoist stiffness is given; each hoist's level sensitivity shows
  the effect of a hoist 1/4 in out of level, one hoist at a time.
...
- Truss data comes from the original workbook's tables or from the manufacturer's own current table (models
  marked MFG); hoist and fixture data come from the original workbook. Always confirm against the manufacturer's
...
| `src/engine/` | Calculations, with no DOM: `beam.js` (continuous beam), `rig.js` (load path), `grillage.js` (whole-rig analysis), `section.js` (truss stiffness from the tables), `limits.js` (table and hoist checks), `side.js` (circular truss and simple UDL calculators) |
...
| `tools/` | `build.py` (single-file build), `serve.py` (dev server), `corner_blocks.py` (corner block data), `pynite_check.py` (cross-check the whole-rig analysis in PyNite) |
...
Truss Grid Analyzer is by **G.E. Simmons Falk**. It is an expansion of *Truss Load Analyzer - EOT* by
